 ...
 * accepted when Apple actually signed it. Reading the payload without checking
 * the signature would let any caller mint their own entitlements, since the
 * payload is nothing more than base64 the caller controls.
 */
class AppleJwsDecoder
{
    /**
     * Apple's chains are [leaf, intermediate, root]. Anything else is not a
     * shape this verifier understands and is refused rather than guessed at.
     */
    ...
    public function verify(string $jws): array
    {
        ...
        if ($algorithm !== 'ES256') {
            // Refusing anything else blocks algorithm-substitution attacks, in
            // which a caller re-signs with a weaker algorithm of their choosing.
    ...
    private function verifyCertificateChain(array $chain): string
    {
        ...
        // The presented root must be the pinned Apple root itself, not merely a
        // self-signed certificate claiming Apple's name. Without this the chain
        // would validate against any root an attacker chose to supply.
    ...
    /**
     * Convert a base64 DER certificate from the x5c header into an OpenSSL
    ...
    {
        // Configurable so tests can pin a root they control and prove that a
        // correctly chained payload is accepted, not only that forgeries fail.
        // A guard that rejects everything would otherwise look identical.
